...
public class RemoveSynchronizeParticipantAction extends Action {
	...
	private boolean promptToSave(List dirtyModels) {
        if (dirtyModels.size() == 1) {
            ...
            ListSelectionDialog dlg = new ListSelectionDialog(view.getSite().getShell(), dirtyModels, new ArrayContentProvider(), new WorkbenchPartLabelProvider(), TeamUIMessages.RemoveSynchronizeParticipantAction_4);
