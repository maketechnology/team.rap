	...
	public static boolean promptToSaveChanges(final Shell shell, final SaveableComparison saveable, final boolean allowCancel) throws InterruptedException {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				String[] options;
				if (allowCancel) {
                    options = new String[] {IDialogConstants.get().YES_LABEL, IDialogConstants.get().NO_LABEL, IDialogConstants.get().CANCEL_LABEL};
				} else {
                    options = new String[] {IDialogConstants.get().YES_LABEL, IDialogConstants.get().NO_LABEL};
				}
