...
/**
 * An abstract compare and merge viewer with two side-by-side content areas and an optional content
 * area for the ancestor. The implementation makes no assumptions about the content type.
 * <p>
 * <code>ContentMergeViewer</code>
 * <ul>
 * <li>implements the overall layout and defines hooks so that subclasses can easily provide an
 * implementation for a specific content type,
 ...
 * <li>tracks the dirty state of the left and right sides and send out notification on state
 * changes.
 * </ul>
 * A <code>ContentMergeViewer</code> accesses its model by means of a content provider which must
 * implement the <code>IMergeViewerContentProvider</code> interface.
 * </p>
 * <p>
 * Clients may wish to use the standard concrete subclass <code>TextMergeViewer</code>, or define
 * their own subclass.
 *
...
public abstract class ContentMergeViewer extends ContentViewer
	...
	/**
     * Copies the content of one side to the other side. Called from the (internal) actions for
     * copying the sides of the viewer's input object.
     *
     * @param leftToRight if <code>true</code>, the left side is copied to the right side; if
     *        <code>false</code>, the right side is copied to the left side
     */
     ...
     *
     * Subclasses may extend this method and use either the toolbar manager provided by the
     * inherited method by calling super.getToolBarManager(parent) or provide an alternate toolbar
     * manager.
     *
		...
		if (!isSaving() && (isLeftDirty() || isRightDirty())) {
			...
			if (Utilities.RUNNING_TESTS) {
				...
				MessageDialog dialog= new MessageDialog(shell,
					...
					MessageDialog.QUESTION,
					new String[] {
                                                IDialogConstants.get().YES_LABEL, // 0
                                                IDialogConstants.get().NO_LABEL, // 1
					},
					0);		// default button index
