...
class ResourceCompareInput extends CompareEditorInput {
		...
		public SelectAncestorDialog(Shell parentShell, IResource[] theResources) {
			super(parentShell, CompareMessages.SelectAncestorDialog_title,
				...
				MessageDialog.QUESTION,
                            new String[] {IDialogConstants.get().OK_LABEL, IDialogConstants.get().CANCEL_LABEL}, 0);
	...
	public Object prepareInput(IProgressMonitor pm) throws InvocationTargetException {
		...
		try {
            // fix for PR 1GFMLFB: ITPUI:WIN2000 - files that are out of sync with the file system
            // appear as empty
	...
	public Object getAdapter(Class adapter) {
		if (IFile.class.equals(adapter)) {
            ...
            if (files != null && files.length > 0)
                return files[0]; // can only return one: limitation on IDE.saveAllEditors; see
                                 // #64617
            return null;
		}
		if (IFile[].class.equals(adapter)) {
